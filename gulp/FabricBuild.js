...
function fabricNuke(done) {
  Plugins.del.sync([
    Config.paths.distCSS,
    Config.paths.distSass,
    Config.paths.temp,
  ]);
  done();
}
...
function fabricCopyAssets() {
  return gulp
    .src([
      Config.paths.srcSass + "/**/*",
      Config.paths.srcSass + "/Fabric.Scoped.scss",
    ])
...
function fabricBuildStyles() {
  var fabric = gulp
    .src(BuildConfig.srcPath + "/" + "Fabric." + BuildConfig.fileExtension)
    .pipe(Plugins.plumber(ErrorHandling.onErrorInPipe))
    .pipe(
      Plugins.gulpif(
        Config.debugMode,
  ...
  var fabricScoped = gulp
    .src(
      BuildConfig.srcPath + "/" + "Fabric.Scoped." + BuildConfig.fileExtension
    )
    .pipe(
      Plugins.gulpif(
        Config.debugMode,
        Plugins.debug({
          title:
            "Building Core Fabric Scoped " +
            BuildConfig.fileExtension +
            " File",
        })
      )
    )
