...
module.exports.reporter = function (options) {
  ...
  function reportFile(filepath, data) {
    ...
    // Report Filename
    lines.push(
      Plugins.colors.white.bold.underline(path.relative(appRoot.path, filepath))
    );
    // Loop through and report warnings
    warningLines.forEach(function (warningLine) {
      var line =
        Plugins.colors.grey("  line " + data[warningLine][0].line) +
        "\t\t  " +
        Plugins.colors.cyan(data[warningLine][0].message);
      lines.push(line);
    });
    lines.push("");
    lines.push(
      "  " +
        ...
        " " +
        Plugins.colors.blue(
          warningCount + " whitespace warning" + (warningCount !== 1 ? "s" : "")
        )
    );
  ...
  return es
    ...
    .on("end", function () {
      ...
      if (totalWarningCount && opts.breakOnWarning) {
        this.emit(
          "error",
          new Plugins.pluginError("lintspaces", "Linter warnings occurred!")
        );
      }
    });
};
