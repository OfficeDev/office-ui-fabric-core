...
function documentationBuildStyles() {
  return gulp
    .src(Config.paths.srcDocumentationCSS + "/" + "docs.scss")
    .pipe(Plugins.plumber(ErrorHandling.onErrorInPipe))
    .pipe(
      Plugins.debug({
        title:
          "Building Documentation SASS " + BuildConfig.fileExtension + " File",
      })
    )
    .pipe(Plugins.header(Banners.getBannerTemplate(), Banners.getBannerData()))
    .pipe(Plugins.header(Banners.getCSSCopyRight(), Banners.getBannerData()))
    .pipe(
      BuildConfig.processorPlugin().on("error", BuildConfig.compileErrorHandler)
    )
...
function prepareHandlebars(cb) {
  ...
  for (var i = 0; i < modelFiles.length; i++) {
    jsonFile = fs.readFileSync(
      Config.paths.srcDocumentationModels + "/" + modelFiles[i],
      "utf8"
    );
...
function documentationPages() {
  return gulp
    .src(Config.paths.srcDocsPages + "/**/index.html")
    .pipe(Plugins.plumber(ErrorHandling.oneErrorInPipe))
    .pipe(
      Plugins.debug({
        title:
          "Building Documentation Page File to " +
          Config.paths.distDocumentation,
      })
    )
...
function documentationIndexPage() {
  return gulp
    .src(Config.paths.srcTemplate + "/documentation-template.html")
    .pipe(Plugins.plumber(ErrorHandling.oneErrorInPipe))
    .pipe(
      Plugins.debug({
        title:
          "Building Documentation File to " + Config.paths.distDocumentation,
      })
    )
    .pipe(Plugins.handlebars(templateData, hbsoptions))
    .pipe(
...
exports.documentationBuild = gulp.series(
  prepareHandlebars,
  gulp.parallel(
    documentationPages,
    documentationBuildStyles,
