...
var ErrorHandling = function () {
  ...
  this.generateBuildError = function (error) {
    Plugins.log(
      Plugins.colors.red("Build error: ") + Plugins.colors.yellow(error)
    );
    return;
  };
  /**
  ...
  this.showNumberOfErrors = function (numberOfErrors) {
    Plugins.log(
      Plugins.colors.red(
        "Number of errors: " + Plugins.colors.yellow(numberOfErrors)
      )
    );
    return;
  };
  ...
  this.showNumberOfWarnings = function (numberOfErrors) {
    Plugins.log(
      Plugins.colors.yellow(
        "Number of warnings: " + Plugins.colors.yellow(numberOfErrors)
      )
    );
    return;
  };
  ...
  this.showSuccessBuild = function (successMessage) {
    Plugins.log(
      Plugins.colors.magenta(
        "Successful build: " + Plugins.colors.green(successMessage)
      )
    );
    return;
  };
  ...
  this.generatePluginError = function (pluginName, errorMessage) {
    var gulpError = new Plugins.pluginError(pluginName, errorMessage, {
      showStack: false,
    });
   ...
   */
  this.createLineErrorMessage = function (
    messageStart,
    path,
    line,
    character,
    ...
    return (
      ...
      ": " +
      Plugins.colors.magenta("line " + line + ", col " + character) +
      ", code " +
      code +
      ", " +
      reason
      ...
      if (lintingData && Object.keys(lintingData).length) {
        ...
        warningLines.forEach(function (warningLine) {
          errorString = that.createLineErrorMessage(
            Plugins.colors.yellow("Warning") +
              " " +
              lintingData[warningLine][0].message,
            file.path,
