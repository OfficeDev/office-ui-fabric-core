...
function csscomb() {
  ...
  var stream = through.obj(function (file, enc, cb) {
    if (file.isNull()) {
      ...
      this.emit(
        "error",
        new Plugins.pluginError(PLUGIN_NAME, "Streams are not supported!")
      );
      return cb();
    } else if (
      file.isBuffer() &&
      ...
      var that = this;
      var configPath = Comb.getCustomConfigPath(
        path.join(path.dirname(file.path), ".csscomb.json")
      );
        ...
        this.emit(
          ...
          new Plugins.pluginError(
            ...
            "Configuration file not found: " +
              Plugins.colors.magenta(configPath)
          )
        );
        return cb();
      }
      ...
      comb
        ...
        .catch(function (err) {
          that.emit(
            "error",
            new Plugins.pluginError(PLUGIN_NAME, file.path + "\n" + err)
          );
        });
    }
    this.push(file);
